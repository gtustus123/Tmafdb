...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.species.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.species.update", [$species->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="code">{{ trans('cruds.species.fields.code') }}</label>
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $species->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('kingdom') ? 'is-invalid' : '' }}" type="text" name="kingdom" id="kingdom" value="{{ old('kingdom', $species->kingdom) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('taxon') ? 'is-invalid' : '' }}" type="number" name="taxon" id="taxon" value="{{ old('taxon', $species->taxon) }}" step="1" required>
                @if($errors->has('taxon'))
                    <div class="invalid-feedback">
                        {{ $errors->first('taxon') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('common_name') ? 'is-invalid' : '' }}" type="text" name="common_name" id="common_name" value="{{ old('common_name', $species->common_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('official_name') ? 'is-invalid' : '' }}" type="text" name="official_name" id="official_name" value="{{ old('official_name', $species->official_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('flag') ? 'is-invalid' : '' }}" type="number" name="flag" id="flag" value="{{ old('flag', $species->flag) }}" step="1" required>
                @if($errors->has('flag'))
                    <div class="invalid-feedback">
                        {{ $errors->first('flag') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sequences') ? 'is-invalid' : '' }}" name="sequences[]" id="sequences" multiple required>
                    @foreach($sequences as $id => $sequences)
                        <option value="{{ $id }}" {{ (in_array($id, old('sequences', [])) || $species->sequences->contains($id)) ? 'selected' : '' }}>{{ $sequences }}</option>
                    @endforeach
                </select>
