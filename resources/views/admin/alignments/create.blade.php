...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.alignment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.alignments.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('seq_1') ? 'is-invalid' : '' }}" name="seq_1_id" id="seq_1_id" required>
                    @foreach($seq_1s as $id => $entry)
                        <option value="{{ $id }}" {{ old('seq_1_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('seq_1'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('seq_2') ? 'is-invalid' : '' }}" name="seq_2_id" id="seq_2_id" required>
                    @foreach($seq_2s as $id => $entry)
                        <option value="{{ $id }}" {{ old('seq_2_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('seq_2'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('alignment') ? 'is-invalid' : '' }}" name="alignment" id="alignment" required>{{ old('alignment') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('identity') ? 'is-invalid' : '' }}" type="number" name="identity" id="identity" value="{{ old('identity', '0') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pair') ? 'is-invalid' : '' }}" type="number" name="pair" id="pair" value="{{ old('pair', '0') }}" step="1" required>
                @if($errors->has('pair'))
                    <div class="invalid-feedback">
                        {{ $errors->first('pair') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gap_1') ? 'is-invalid' : '' }}" type="number" name="gap_1" id="gap_1" value="{{ old('gap_1', '0') }}" step="1" required>
                @if($errors->has('gap_1'))
                    <div class="invalid-feedback">
                        {{ $errors->first('gap_1') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gap_2') ? 'is-invalid' : '' }}" type="number" name="gap_2" id="gap_2" value="{{ old('gap_2', '0') }}" step="1" required>
                @if($errors->has('gap_2'))
                    <div class="invalid-feedback">
                        {{ $errors->first('gap_2') }}
                    </div>
