    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.regions.update", [$region->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('identifier') ? 'is-invalid' : '' }}" name="identifier_id" id="identifier_id" required>
                    @foreach($identifiers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('identifier_id') ? old('identifier_id') : $region->identifier->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('localisation_type') ? 'is-invalid' : '' }}" name="localisation_type_id" id="localisation_type_id" required>
                    @foreach($localisation_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('localisation_type_id') ? old('localisation_type_id') : $region->localisation_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('region_source') ? 'is-invalid' : '' }}" name="region_source_id" id="region_source_id" required>
                    @foreach($region_sources as $id => $entry)
                        <option value="{{ $id }}" {{ (old('region_source_id') ? old('region_source_id') : $region->region_source->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('start') ? 'is-invalid' : '' }}" type="number" name="start" id="start" value="{{ old('start', $region->start) }}" step="1" required>
                @if($errors->has('start'))
                    <div class="invalid-feedback">
                        {{ $errors->first('start') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('end') ? 'is-invalid' : '' }}" type="number" name="end" id="end" value="{{ old('end', $region->end) }}" step="1" required>
                @if($errors->has('end'))
                    <div class="invalid-feedback">
                        {{ $errors->first('end') }}
                    </div>
