...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.identifier.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.identifiers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('database') ? 'is-invalid' : '' }}" name="database_id" id="database_id" required>
                    @foreach($databases as $id => $entry)
                        <option value="{{ $id }}" {{ old('database_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sequence') ? 'is-invalid' : '' }}" name="sequence_id" id="sequence_id" required>
                    @foreach($sequences as $id => $entry)
                        <option value="{{ $id }}" {{ old('sequence_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
